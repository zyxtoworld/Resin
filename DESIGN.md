...
* 接入：代理地址与访问方式。
* 策略：编辑平台策略、过滤与响应规则并保存。
* 路由状态：展示平台节点运行态、平台隔离的内存冷却和稳定游标分页租约；日常租约释放与清空在此完成。
* 危险操作：仅保留重置配置和删除平台。

路由状态租约游标绑定排序、方向、筛选和页大小；篡改、截断、参数不匹配以及无法验证的游标均返回 HTTP 400。当前 Resin 的 Compose 部署是单个有状态进程/容器，路由内存态与本地 SQLite 不在副本间共享，因此游标签名密钥按进程生成，重启后旧游标会失效；WebUI 收到带游标请求的 400 后自动回到第一页并重新读取。多副本部署不属于当前支持合同。
